...
Treat `AGENTS.md` as a compatibility entrypoint for tools that look for repo-level agent instructions. The canonical rules still live in `REPO.md`.
...
- `logs/` is a legacy archive. Keep existing references working, but put all new durable research, decisions, and worklogs into the active repo-template surfaces.
