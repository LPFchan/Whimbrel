...
if [ -n "$expected_project_id" ] && [ "$project" != "$expected_project_id" ]; then
  fail "project trailer must match project-id: $expected_project_id"
fi
...
echo "$artifacts" | grep -Eq '^(IBX|RSH|DEC|LOG|UPS)-[0-9]{8}-[0-9]{3}(, (IBX|RSH|DEC|LOG|UPS)-[0-9]{8}-[0-9]{3})*$' ||
